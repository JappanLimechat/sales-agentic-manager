...
export function DashboardShell(props: Props) {
  const { view, onViewChange, query, onQueryChange, region, onRegionChange, timeRange, onTimeRangeChange, records } = props;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-semibold leading-none text-pretty">
                  <span className="text-foreground">LimeChat's</span> <span className="text-muted-foreground">Ai Sales Agentic Manager (SAM)</span>
                </h1>
          ...
          <div className="flex flex-col gap-3 px-4 pb-3">
            <div className="flex flex-wrap items-center gap-3">
              <div className="min-w-[220px]">
                <Input value={query} onChange={e => onQueryChange(e.target.value)} placeholder="Search POCs/companies..." />
              </div>
              ...
              <Select value={view} onValueChange={v => onViewChange(v as (typeof Views)[number])}>
                <SelectTrigger className="w-[180px] bg-white text-black border-2 border-[#667eea] shadow-lg hover:shadow-xl transition-all duration-200 hover:bg-[#5a6de0] focus:border-[#667eea] focus:ring-2 focus:ring-[#667eea]/20">
                  <SelectValue placeholder="View" className="font-medium" />
                </SelectTrigger>
                <SelectContent className="border-2 border-[#667eea]/20 shadow-xl rounded-lg overflow-hidden">
                  {Views.map(v => (
                    <SelectItem key={v} value={v} className="hover:bg-[#667eea]/10 focus:bg-[#667eea]/20 transition-colors duration-150 font-medium">
                      {v} View
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        <table className="w-full text-sm">
          <thead className="text-xs text-muted-foreground">
            <tr className="border-b">
              ...
              <Th onClick={() => toggleSort('dealSizeINR')} label="Deal Size (₹)" active={sortKey === 'dealSizeINR'} dir={sortDir} />
              ...
              <Th onClick={() => toggleSort('sentiment')} label="Sentiment" active={sortKey === 'sentiment'} dir={sortDir} />
...
function Th({ label, active, dir, onClick }: { label: string; active: boolean; dir: 'asc' | 'desc'; onClick: () => void }) {
  return (
    <th
      role="button"
      onClick={onClick}
      className={cn('py-2 px-2 text-left select-none', active ? 'text-foreground' : 'text-muted-foreground', 'hover:text-foreground')}
...
function IndustryInsights() {
  ...
  const grouped = React.useMemo(() => {
    const map = new Map<POCRecord['sector'], { sector: POCRecord['sector']; total: number; avgDeal: number; avgStage: number }>();
            ...
            return (
              <div key={g.sector} className="rounded-lg border p-3">
                ...
                <div className="mt-2 h-1.5 w-full rounded-full bg-muted">
                  <div className="h-1.5 rounded-full bg-[#667eea]" style={{ width: `${conversionRate}%` }} aria-hidden />
                </div>
              </div>
            );
          })}
